...
public class TabPanel extends Panel {
	...
	Vector<String> names = new Vector<String>(10, 10); // contains the (interned) card names
	...
	public boolean handleEvent(Event e) {
		if (e.id == Event.MOUSE_DOWN) {
			...
			if (applet != null) {
				int i = cardAt(e.x, e.y);
				if (i != -1)
					applet.showStatus(documentCard((String) names
							.elementAt(i)));
			}
	...
	void computeTabs() {  // Compute positions of the tabs.
		...
		for (int i = 0; i < nCards; i++) { // size the tabs & reshape the panes.
			pos[i] = x;
			width[i] = tabH
					+ metric.stringWidth((String) names.elementAt(i));
			x += width[i];
		}
		pos[nCards] = x;
		@SuppressWarnings("deprecation")
	...
	void paintTabEdge(Graphics g, int x, int edges[][]) {
		...
		for (int i = 0; i < tabN - 2; i++)
			g.drawLine(edges[0][i], edges[1][i], edges[0][i + 1],
					edges[1][i + 1]);
		g.translate(-x, 0);
	}
